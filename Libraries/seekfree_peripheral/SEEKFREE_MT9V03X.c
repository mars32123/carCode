...
//必须4字节对齐
IFX_ALIGN(4) vuint8 mt9v03x_image[MT9V03X_H][MT9V03X_W];
...
int16 MT9V03X_CFG[CONFIG_FINISH][2] = {
    ...
    {LR_OFFSET,
     -1}, //图像左右偏移量    正值 右偏移   负值 左偏移  列为188 376
          //752时无法设置偏移
          //摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
    {UD_OFFSET,
     120}, //图像上下偏移量    正值 上偏移   负值 下偏移  行为120 240
           //480时无法设置偏移
           //摄像头收偏移数据后会自动计算最大偏移，如果超出则设置计算出来的最大偏移
...
{
    MT9V03X_CFG[1][1]=(int16)SystemData.exp_time;
    MT9V03X_CFG[5][1]=(int16)SystemData.mtv_lroffset;
    MT9V03X_CFG[7][1]=(int16)SystemData.mtv_gain;
	uint8 i;
    ...
    // set_mt9v03x_reg(MT9V03X_COF_UART, 0xaf, 0x303);  // AEC/AGC Enable 0xAF[1:0] 0xAF[9:8]
...
vuint8	mt9v03x_dma_int_num;	//当前DMA中断次数
//-------------------------------------------------------------------------------------------------------------------
//  @brief      MT9V03X摄像头场中断
//  @param      NULL
//  @return     void			
