...
void ips114_fill(uint16 x1,uint16 y1,uint16 x2,uint16 y2, uint16 color) {
    IfxCpu_setSpinLock(&spinLock, 0xffffff);
...
//-------------------------------------------------------------------------------------------------------------------
void ips114_showchar(uint16 x,uint16 y,const int8 dat, uint16 front_color,uint16 back_color)
{
	uint8 i,j;
	uint8 temp;
	...
	for(i=0; i<16; i++)
	{
		...
		for(j=0; j<8; j++)
		{
			if(temp&0x01)	
            {
                ips114_writedata_16bit(front_color);
            }
			else			ips114_writedata_16bit(back_color);
			temp>>=1;
		}
	}
    IfxCpu_resetSpinLock(&spinLock);
...
//-------------------------------------------------------------------------------------------------------------------
void ips114_showstr(uint16 x,uint16 y,const int8 dat[], uint16 front_color,uint16 back_color)
{
	uint16 j;
	...
	while(dat[j] != '\0')
	{
		ips114_showchar(x+8*j,y*16,dat[j], front_color,back_color);
		j++;
	}
}
...
void ips114_showint8(uint16 x,uint16 y,int8 dat)
{
	...
	if(dat<0)
	{
		ips114_showchar(x,y*16,'-',IPS114_PENCOLOR,IPS114_BGCOLOR);
		dat = -dat;
	}
	else	ips114_showchar(x,y*16,' ',IPS114_PENCOLOR,IPS114_BGCOLOR);
	...
	while(i<3)
	{
		ips114_showchar(x+(8*(i+1)),y*16,'0' + a[i],IPS114_PENCOLOR,IPS114_BGCOLOR);
		i++;
	}
}
...
void ips114_showuint8(uint16 x,uint16 y,uint8 dat)
{
	...
	while(i<3)
	{
		ips114_showchar(x+(8*i),y*16,'0' + a[i],IPS114_PENCOLOR,IPS114_BGCOLOR);
		i++;
	}
...
void ips114_showint32(uint16 x,uint16 y,int32 dat,uint8 num)
{
    ...
    ips114_showstr(x, y, buff,IPS114_PENCOLOR,IPS114_BGCOLOR);	//显示数字
}
...
//-------------------------------------------------------------------------------------------------------------------
void ips114_displayimage01(uint8* p,
                           uint16 width,
                           uint16 height) { //显示黑白图像
  uint32 i, j;
  uint16 color = 0;
  uint16 temp = 0;
  uint16 coord_x = 0;
  ...
  for (j = 0; j < coord_y; j++) {
    for (i = 0; i < coord_x; i++) {
      ...
      if (i == 2 * ImageDeal[j / 2].Center ||i == 2 * ImageDeal[j / 2].Center + 1) /*&&
          j >= 2 * ImageStatus.OFFLine*/
        ips114_writedata_16bit(GREEN);
      else if(i == 2 * ImageDeal[j / 2].LeftBorder ||i == 2 * ImageDeal[j / 2].LeftBorder +1)
        ips114_writedata_16bit(RED);
      else if(i == 2 * ImageDeal[j / 2].RightBorder ||i == 2 * ImageDeal[j / 2].RightBorder -1)
        ips114_writedata_16bit(RED);
      else if(i == 2 * ImageDeal[j / 2].mid_temp||i == 2 * ImageDeal[j / 2].mid_temp+1)
          ips114_writedata_16bit(RED);
      else if (j == ImageStatus.OFFLine * 2)
        ips114_writedata_16bit(BLUE);
      else if (j == ImageStatus.TowPoint * 2)
        ips114_writedata_16bit(PURPLE);
...
void ips114_display_image_full_screen(const uint8* p) {
  ips114_set_region(0, 0, IPS114_X_MAX - 1, IPS114_Y_MAX - 1);  //设置显示区域

  for (int j = 0; j < IPS114_Y_MAX; ++j) {
    for (int i = 0; i < IPS114_X_MAX; ++i) {
      if (2 * (j * IPS114_X_MAX + i) + 1 < 40320) {
        uint16 color = p[2 * (j * IPS114_X_MAX + i)];
        color <<= 8;
        color |= p[2 * (j * IPS114_X_MAX + i) + 1];
        ips114_writedata_16bit(color);
      } else
        ips114_writedata_16bit(j << 8 | i);
    }
  }
}
